        ...
        private async Task<string> CreateJsonMessageAsync(MessageData messageData)
{
        ...
        protected async Task MonitoredItemsProcessorAsync(CancellationToken ct)
        {
            ...
            using (hubMessage)
            {
                try
                {
                    ...
                    {
                        hubMessage.Write(Encoding.UTF8.GetBytes("{\"id\":\"556\",\"Records\":["), 0, 23);
                    }
                    while (true)
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                hubMessage.Write(Encoding.UTF8.GetBytes("]}"), 0, 2);
                            ...
                            if (_iotHubClient != null || _edgeHubClient != null)
                            {
                                ...
                                try
                                {
                                    ...
                                    if (_iotHubClient != null)
                                    {
                                        ...
                                        await HubClient.ProcessAsync(encodedhubMessage);
                                    }
                                    else
                                    {
                                ...
                                if (!singleMessageSend)
                                {
                                    hubMessage.Write(Encoding.UTF8.GetBytes("{\"id\":\"559\",\"Records\":["), 0, 23);
                                }
        ...
        private static int MAX_RESPONSE_PAYLOAD_LENGTH = (800 * 1024 - 256);
        private static string CONTENT_TYPE_OPCUAJSON = "application/opcua+uajson";
        private static string CONTENT_ENCODING_UTF8 = "UTF-8";


        // Namff _hubConnectionString = "HostName=IoT2018-Hub.azure-devices.net;DeviceId=device-01;SharedAccessKey=YUyzxO7RjK2/DhxMW58QjGVqH2DS6Vj3bIg+ej66xIo=";
        //Rasmus _hubstring = "HostName=demo-hub-iot.azure-devices.net;DeviceId=device-01;SharedAccessKey=S7+L9d1h0iRhhhzMpYXu9tcZz7zZXNq4Mc8YBfaglbI=";
        private static string _hubConnectionString = "HostName=demo-hub-iot.azure-devices.net;SharedAccessKeyName=iothubowner;SharedAccessKey=u5+BNnCWeKe4ihi0MzDij2/rINiJNvgJCyvsQtIO2+A=";
